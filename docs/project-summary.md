...
Обновлено: 2026-05-22 (добавлен перевод на английский)
...
- последний полный цикл `local commit -> GitHub push -> VPS fetch/reset -> systemd restart` проверен 2026-05-22;
...
- переводит расшифровку на разговорный английский через Groq (Llama 3.3 70B);
...
- реальные значения из локального `.env` проверялись поиском по Git history и не найдены.
- `GROQ_API_KEY` добавлен в локальный `.env` и в `/etc/pqh8temc_bot/pqh8temc_bot.env` на VPS.
...
Следующий смысловой этап: дальнейшее расширение бизнес-логики бота по запросу пользователя.
...
`GROQ_API_KEY` — бесплатный API-ключ от console.groq.com. Используется для перевода транскрипта на английский через модель `llama-3.3-70b-versatile`.
...
- Отправляет аудио в Deepgram, использует модель `nova-3-general`.
- Включено автоопределение языка через `detect_language=true`, поэтому можно говорить, например, по-русски или по-английски.
- После расшифровки переводит текст на разговорный английский через Groq (модель `llama-3.3-70b-versatile`).
- Ответ бота состоит из двух блоков: `Транскрипт:` (оригинал) и `English:` (перевод).
...
- Если перевод не удался, бот всё равно отдаёт транскрипт с сообщением об ошибке.
...
- публичный GitHub-репозиторий, локальная ветка, `origin/main` и checkout на VPS синхронизированы через деплой-скрипт;
- голосовые сообщения распознаются через Deepgram;
- язык распознавания определяется автоматически;
- после транскрипции текст переводится на разговорный английский через Groq (Llama 3.3 70B);
